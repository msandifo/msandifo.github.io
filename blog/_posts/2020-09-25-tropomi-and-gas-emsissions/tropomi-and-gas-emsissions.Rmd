...
ndg.q <- ndg.s %>% mutate(year=year(date), quarter=quarter(date)) %>% 
  group_by (year, quarter) %>%
  summarise(date=mean(date), flare.mmcf=mean(flare.mmcf))

pfr <- pfr %>% mutate(quarter=quarter(date), year=year(date))
pf.ndg <-full_join(pfr,ndg.q, by =c("year","quarter"))[, c(3,5,4,2,7)] %>%
  rename(date=date.x, permian=flare.mmcf.x, bakken=flare.mmcf.y) %>%
  pivot_longer(cols=c(permian, bakken))
pf.ndg.y <- pf.ndg %>% group_by(year, name) %>%
  summarise(date=mean(date), value=mean(value)) %>% subset(year<2020)

ggplot(pf.ndg, aes(date, value, col=name))+
  geom_line(size=.3)+
    geom_label(data=pf.ndg.y %>% subset(year>2016  ), aes(label= paste0(round(value)," MMcf")),size=2.85, show.legend=F, hjust=-.05, vjust=-.1,  label.size=0 )+
   geom_label(data=pf.ndg.y %>% subset(year>2016  ), aes(label= paste0(  round(value* mmcf2mt * 365,1)," Mt")), size=2.85, show.legend=F, hjust=-.05, vjust=.8,  label.size=0)+
   geom_line(size=.3, alpha=.3)+
   geom_point(data=pf.ndg.y %>% subset(year>2016  ), 
                size=2.5, show.legend=F, colour="white")+

   geom_point(data=pf.ndg.y %>% subset(year>2016  ), 
                size=1.85, show.legend=F)+

#  geom_point(shape=5, size=2.)+
  #   geom_line(data=ndg.q, col="red3", linetype=1, size=.2)+
...
#  geom_point(size=.2)+
  ...
  theme(legend.position=c(.13,.8), legend.title = element_blank())+
